def identify_hand(hand):
	...
	four = 0
	five = 0
	# print(max(countOfChars))
	countOfChars[countOfChars.index(max(countOfChars))] += wildcard
	# print(countOfChars[countOfChars.index(max(countOfChars))])
	# print(max(countOfChars))
	...
	for i, next_hand in enumerate(sorted):
		...
		for j, char in enumerate(hand):
			...
			if char == next_hand[0][j]:
				continue
			if char.isdigit() and ((next_hand[0][j].isdigit() and char > next_hand[0][j]) or next_hand[0][j] == "J"):
				# print("HERE6")
				return i
			...
			# 	# print("HERE2")
			# 	return i
			elif char == "T" and (next_hand[0][j].isdigit() or next_hand[0][j] == "J"):
				# print("HERE")
				return i
			break 
	# print("HUH")
